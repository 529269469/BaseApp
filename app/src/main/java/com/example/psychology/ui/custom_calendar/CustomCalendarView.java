...
public class CustomCalendarView extends FrameLayout {
    ...
    public void setCalendarDataList(List<CalendarData> calendarData) {
    ...
    private void initAdapter() {
        mAdapter = new BaseRecycerAdapter<CalendarBean>(getContext(), mDatas, R.layout.item_scheduling_day) {
            ...
            public void onBindViewHolder(ViewHolder viewHolder, CalendarBean itemVO, int position) {
                ...
                if (itemVO.getDayType() == 1||itemVO.getDayType() == 2) {
                    ...
                    tv_type.setVisibility(GONE);
                    tv_state.setVisibility(GONE);
                    String day=itemVO.getDay();
                    for (int i = 0; i < calendarDataList.size(); i++) {
                        if (day.equals(calendarDataList.get(i).getDay())){
        ...
        mAdapter.setOnItemClickListener((o, position) -> {
            CalendarBean calendarBean = mDatas.get(position);
            if (listener != null) {
                if (calendarBean.getDayType() == 1||calendarBean.getDayType() == 2) {

                }else {
                    listener.onDay(calendarBean.getDay());
                }
